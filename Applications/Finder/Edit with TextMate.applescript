...
on fail(_message)
	tell application "System Events" to display alert "Error" message _message as warning buttons {"OK"} default button 1
	return
end fail
...
on process(argv)
	...
	if (count of _files) is less than 1 then return my fail("Nothing selected")
	
	set _pathnames to ""
	repeat with f in _files
		set _pathnames to _pathnames & " " & quoted form of POSIX path of (f as alias)
	end repeat
	if _pathnames is "" then return my fail("No valid paths selected")
